...
외국인 노동자가 생활정보·비자·거주 등을 공유하는 게시판. 게시글·댓글·좋아요·신고·번역, 인증 배지, 이웃 온도를 제공한다.
...
- 사용자는 **게시글을 작성·수정·삭제**할 수 있다(이미지 첨부 가능).
- 사용자는 게시글에 **댓글·대댓글**을 달 수 있다.
- 사용자는 게시글/댓글에 **좋아요**를 누르고, **관심글 목록**을 볼 수 있다.
- 사용자는 부적절한 게시글/댓글을 **신고**할 수 있다(중복 신고 불가).
- 사용자는 외국어 게시글을 **번역 보기** 할 수 있다.
- 게시글 카드/상세에 작성자의 **인증 배지**와 **이웃 온도**가 표시된다.
- 사용자는 함께 거주한 회원의 **이웃 온도를 평가**할 수 있다(자기 평가·중복 평가 불가).
...
### 인증 배지
- 신분증 인증 완료(`users.is_verified = TRUE`) 회원은 닉네임 옆에 배지 표시.
- 응답 필드: `author_is_verified`(boolean).

### 친절한 이웃 온도 (`temperature_grade`)
- 함께 거주한 사람들의 평가(1~5점)로 산출한 신뢰도를 **색상 등급**으로 표현.
...
- 평가 점수 평균(`average_score`, 1.0~5.0, 표시용 number)을 등급으로 매핑.
- 평가 저장 시 대상 회원의 `temperature_grade` 재집계.

---

## 5. 신고 사유 (`reason`)

`SPAM` / `INAPPROPRIATE` / `MISINFORMATION` / `HATE` / `OTHER`

- 동일 게시글/댓글 중복 신고 불가(중복 시 `COMMON4091`).
- 신고 누적 처리(블라인드 등)는 운영 정책으로 별도.

> 신고용 테이블(reports)은 ERD에 신규 추가가 필요하다(팀 확정 사항).
...
- `posts`, `comments`, `likes`, `user_reviews` (+ 신고용 reports 신규)
