...
| AI (계정 B) | **AWS Bedrock (Claude)** + **Bedrock Knowledge Bases** (법령 RAG, 백엔드 = S3 Vectors) | 분석: OCR/분석/번역(문서) · 후속 챗봇: Tool Use 루프 + 도구 4종 (법령=KB / 환율·커뮤니티=자체 MCP1·2 / 웹검색=외부 Tavily Remote MCP 직결). 법령 검색은 분석·챗봇 공통으로 KB `retrieve`. **커뮤니티 번역(게시글·댓글)** 도 같은 Bedrock Claude(Haiku 단발 호출)를 공유 — 별도 번역 Lambda로 분리, lazy 호출, [`community/translation.md`](./community/translation.md) |
