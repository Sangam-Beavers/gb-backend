...
>
> ⚠️ `AUTH4004`는 현재 "이메일 인증 미완료(로그인, 403)"와 "유효하지 않은 리프레시 토큰(재발급, 401)" 두 의미가 겹쳐 있다. HTTP 상태와 호출 맥락(로그인 vs 재발급)이 달라 당장은 구분되지만, 엄밀히 하려면 둘 중 하나를 `AUTH4007` 등 새 번호로 분리 권장. (이번 정리에서는 우선순위가 높은 AUTH4005만 확정 분리)
