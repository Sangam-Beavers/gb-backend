...
- **예외:** 금액·환율이 아닌 **표시용 수치**(계산에 쓰지 않는 값)는 `number` 허용.
  - 등락률 `change_rate`, OCR 신뢰도 `ocr_confidence`, 매너 온도 `author_temperature` 등
...
| code | HTTP | 의미 |
| --- | --- | --- |
| `AUTH4001` | 401 | 이메일 또는 비밀번호가 올바르지 않습니다. |
| `AUTH4002` | 401 | (예약) 액세스 토큰 무효 — 시드 점유. 신규 사용 금지 |
| `AUTH4003` | 401 | (예약) 액세스 토큰 만료 — 시드 점유. 신규 사용 금지 |
| `AUTH4004` | 403 / 401 | 이메일 인증 미완료 계정(로그인 403) / 유효하지 않은 리프레시 토큰(재발급 401) |
...
>
> ⚠️ `AUTH4004`는 현재 "이메일 인증 미완료(로그인, 403)"와 "유효하지 않은 리프레시 토큰(재발급, 401)" 두 의미가 겹쳐 있다. HTTP 상태와 호출 맥락(로그인 vs 재발급)이 달라 당장은 구분되지만, 한 코드가 두 HTTP(403/401)를 갖는 건 ErrorCode enum이 `getHttpStatus()`를 하나만 반환하는 구조와 충돌한다. **인증 구현 시점에 반드시 분리한다(액션 확정):** "이메일 인증 미완료(403)"는 `AUTH4004`로 유지, "유효하지 않은 리프레시 토큰(401)"을 신규 `AUTH4007`로 분리한다. (한 번 확정한 번호는 재배치 금지 원칙에 따라, 기존 의미 중 다수 호출 경로인 이메일 미완료를 4004에 남긴다.) 현재 인증 미구현 단계에서는 두 코드 모두 사용처가 없으므로 지금 당장의 코드 변경은 없다.
...
| `WALLET4002` | 400/422 | 지갑 잔액이 부족합니다. |

### 송금 (TRANSFER)
| code | HTTP | 의미 |
| --- | --- | --- |
| `TRANSFER4001` | 404 | 존재하지 않는 송금 내역입니다. |
...
| `ACCOUNT4005` | 429 | 인증 요청 횟수를 초과했습니다. 잠시 후 다시 시도해주세요. |
...
| `temperature_grade` | `RED` / `YELLOW` / `GREEN` / `PURPLE` / `BLUE` |
