...
- **충전/출금 구현 = 수준 2 (Mock API).** 실제 PG/은행 연동 대신 동일 인터페이스의 외부 Mock 은행 서버(Beaver/Quokka Bank)로 처리한다. 본체는 `BankClient` 인터페이스로 호출하고 실서비스 전환 시 URL/구현체만 교체. 충전은 `withdrawal`(외부계좌 차감), 현금화는 `payout`(외부계좌 증액). 계좌 등록 시 본체가 은행 `verify`를 재호출해 발급받은 `account_token`을 `bank_accounts.mock_account_token`에 저장해 충전 때 사용(클라가 보낸 토큰은 신뢰·저장 안 함 — api-spec §13-2). 상세 연동·에러 매핑: [`remittance/api-spec.md`](./remittance/api-spec.md) §13.
