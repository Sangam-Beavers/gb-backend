...
> **지원 국적·언어 스코프 (MVP 확정)** — 국적 4개: 한국(`KR`) · 미국(`US`) · 베트남(`VN`) · 필리핀(`PH`). 언어 4개(BCP 47): `ko` · `en` · `vi` · `fil`. 신분증 인증 4개국(#108) 및 통화 4개(KRW/USD/PHP/VND)와 1:1 정렬. DB 컬럼·DTO 자체는 `VARCHAR(10)` 자유문자열로 유지(확장 여지)하며 화이트리스트 검증은 프론트 select 박스로 강제. Phase 2 재편 계획은 결정문(`심규보/결정-국적언어-스코프-확정.md`) 참조.
