 ...
 *
 * <p>{@code documentNumber}는 명세상 <b>AES-256 암호화 저장</b> 대상이나, 암복호 유틸(D-3) 미도입으로
 * 현재는 데모 한정 평문 저장한다(서비스 {@code submitVerification}의 {@code // TODO} 참고).
 * 유틸 도입 시 저장/조회 변환 지점만 교체하면 된다.
 */
@Entity
...
public class UserVerification extends BaseEntity {
    ...
    /** 문서 번호. TODO(D-3): AES-256 암호화 저장으로 교체(현재 데모 평문). */
