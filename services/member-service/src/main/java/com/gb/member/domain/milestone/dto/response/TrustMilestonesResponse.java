 ...
 * "다음에 뭘 하면 되는지"(진척도 + CTA)를 그릴 수 있게 하기 위함(이슈 BE-6 §Etc). Phase 2 카탈로그 3종:
 * ID_VERIFIED(합성 — members.is_verified) / BANK_ACCOUNT_CONNECTED / FIRST_TRANSACTION_COMPLETED
 * (member_milestones). 카탈로그 순서는 등급 순차 체인(Lv2→Lv3→Lv4) 순서로 고정.
 *
...
public class TrustMilestonesResponse {
    ...
    @Schema(description = "현재 신뢰등급(이슈 #193 — Phase 2 확장)",
            allowableValues = {"NEWCOMER", "VERIFIED", "CONNECTED", "TRUSTED"}, example = "CONNECTED")
    ...
    public static class MilestoneItem {
        ...
        @Schema(description = "마일스톤 종류. ID_VERIFIED=신분증 인증(합성), 나머지는 member_milestones 기록",
