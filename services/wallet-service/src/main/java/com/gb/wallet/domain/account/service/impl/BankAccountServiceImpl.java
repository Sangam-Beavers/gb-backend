...
public class BankAccountServiceImpl implements BankAccountService {
    ...
    public AccountResponse registerAccount(String userPublicId, RegisterAccountRequest request) {
        // TODO: race-safety — 두 등록 요청이 동시에 existsBy → false → 둘 다 INSERT 가능.
        //       해결책: bank_accounts에 (user_public_id, bank_id, account_number) UNIQUE 제약 또는
        //       Redis 분산 락(SET lock:account-register:{user}:{bank}:{acct} NX EX 5). 명세에 UNIQUE 정의가
        //       없으므로 별도 마이그레이션 이슈에서 도입한다.
