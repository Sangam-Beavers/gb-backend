...
import com.gb.document.global.event.MilestoneAchievedEvent;
...
/**
 * Kafka Producer 설정 (Phase 3 — 마일스톤 이벤트 발행, BE-7. wallet KafkaProducerConfig 패턴 복제).
 *
...
public class KafkaProducerConfig {
     ...
     * AFTER_COMMIT 리스너(MilestoneEventPublisher)는 SQS Consumer 스레드에서 동기 실행되므로, 길게 두면
     * Kafka 장애가 결과 수신 처리 지연(visibility timeout 초과 위험)으로 번진다 — 짧게 캡해 빠르게
     * 실패시키고 ERROR 로그로 넘긴다(발행 실패 = 유실 허용 + 보정, 스파이크 결정 4).
     * 테스트(application-test.yml)는 100ms로 더 줄인다.
     */
